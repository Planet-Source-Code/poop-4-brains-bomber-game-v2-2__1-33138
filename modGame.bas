...
Const sndExplo As Integer = 1 'the sound labels (for the sounds since they're stored in buffers)
...
Public Player As Player 'the player object
Public P(1 To 10) As Enemy 'the plane objects
Public Sh(1 To 10) As Enemy 'the ship objects
Public S(1 To 10) As Enemy 'the submarine objects
Public Ex(1 To 20) As Explosion 'the explosion objects
Public plShot(1 To 20) As Shot 'the plane shot objects
Public ShShot(1 To 20) As Shot 'the ship shot objects
Public B(1 To 10) As Shot 'the bomb objects
Public PShot(1 To 20) As Shot 'the player shot objects
Public E As New ClsEngine 'the game engine for just about everything that has to with drawing and sound
Public Running As Boolean 'the boolean to decide if the game is running or not
Public Message As String 'the big lettered messsage at the beginning
Public Box(1 To 10) As Shot 'not really a shot but the little box's that fall from the top of the screen
Public M(1 To 6) As MenuItem 'the items for the game menu
Public Selected As Long 'what is selected on the game menu
Public Pause As Long 'pause counter for how long the game is paused (no unit)
Public Osama As OsamaShip 'the easter egg (osama bin laden's ship)
Public Sound As New DS_Engine 'sound engine controlled by the game engine
Public Speed As Long 'the speed of the game

Function MakeMenu()
Selected = 1 'set it so that the selected item is New Game

'set the item's props
M(1).CAPTION = "New Game": M(1).FUNC = "new"
M(2).CAPTION = "Top Score": M(2).FUNC = "tscore"
M(3).CAPTION = "Tactics (Help)": M(3).FUNC = "Tact"
M(4).CAPTION = "Options": M(4).FUNC = "PROPS"
M(5).CAPTION = "About": M(5).FUNC = "About"
M(6).CAPTION = "Exit": M(6).FUNC = "Exit"
End Function

Function MakePlane()
Dim I As Long

'put a default plane in a certain spot and make it go
    ...
    If P(I).Act = False Then
        ...
        Select Case Int(Rnd * 2)
            ...
            Case 1
                ...
                P(I).Dir = dRight
        End Select
        Exit For 'exit the loop so only one plane is made
    End If
Next I
End Function
...
'set a default ship in a spot and make it go
...
'set a default sub in a spot and make it go
...
If Player.HP <= 0 Then 'if a player is dead
    Running = False 'stop the game
    Message = "Game Over" 'tell the user game over
    
    If Player.Score > Val(GetSetting("BOMBER2", "TOPSCORE", "SCORE")) Then 'if he or she beat the top score
        SaveSetting "BOMBER2", "TOPSCORE", "SCORE", Player.Score
        'enter a given name into the records
        SaveSetting "BOMBER2", "TOPSCORE", "NAME", InputBox("You have beaten the TOP SCORE!" & vbCrLf & "Please enter your name for the records...", "You Won", "Player")
    End If
    
End If

'reset the player's reload
...
'slow down the player's hor speed
...
'slow down the player's ver speed
...
'detect if a shot hit the ship or not and make the explosions and sound if it did
For I = 1 To 20
...
'reset all of the player's position stats
Player.Dir = dRight
Player.HP = 50
Player.ReloadB = 0
Player.ReloadS = 0
...
Osama.Act = False 'make sure you dont have osama still on your screen

Running = True 'start the game

frmGame.board.SetFocus 'set focus to the game screen
End Function

Function ShipShoot(Index)
'if the ship's gun hasnt reset or the ship doesnt exist then dont let it shoot
If Sh(Index).Reload <> 0 Or Sh(Index).Act = False Then Exit Function

'if the ai lets it shoot
If Player.X > Sh(Index).X - AI_W And Player.X < Sh(Index).X + AI_W Then

'find a ship bullet object and fire it
Dim I As Long
...
'if the plane isnt ready to shoot
...
'if the player isnt ready to shoot then dont shoot
If Player.ReloadS <> 0 Or Player.Act = False Then Exit Function

Dim I As Long
'find a player's bullet and fire it
...
Function PlayerBomb()
'if the player isnt ready to bomb then dont bomb
If Player.ReloadB <> 0 Or Player.Act = False Then Exit Function

Dim I As Long

'find a player's bomb and... drop it
...
'move all of the shots and see if any of the player shots hit any enemies
For I = 1 To 20
...
'check all the buttons for input from the player
...
'set the max number of frames so the person coding dont have it do it (aint i a cheat?)
Frames = frmGame.explo(T).ScaleWidth \ W

'select a explosion thats not being used and use it
For I = 1 To 20
...
'do the animation for the explosions
For I = 1 To 20
...
size = board.FontSize 'set the size to fontsize to the board fontsize so it wont be disturbed
...
For I = 1 To UBound(M())
    ...
    If I = Selected Then board.ForeColor = vbRed 'if the item is selected then highlight it
    board.Print M(I).CAPTION 'print the item's caption
Next I

board.FontSize = size 'reset the fontsize
End Function

Function DoItem()
'depending on the selected item's function
...
Function MakeOsama()
If Osama.Act = True Then Exit Function 'if osama is already out then let him die
Osama.Act = True 'put him out
Osama.X = frmGame.board.ScaleWidth 'make him come out of the right hand side of the game screen
End Function

Function SwitchDir(obj As Enemy)
'switch the direction of an object depending on the player's position
...
Function DropBox()
Dim I As Long
'find a box slot and drop it
For I = 1 To 10
    If Box(I).Act = False Then
        Box(I).Act = True
        Box(I).X = Int(Rnd * (frmGame.board.ScaleWidth \ 20)) * 20
